...
ItemSet.prototype._onAddGroups = function(ids) {
  ...
  ids.forEach(function (id) {
    ...
    if (!group) {
      ...
      //TODO - must be recursive jobrian
...
ItemSet.prototype._orderNestedGroups = function(groupIds) {
  ...
  var treeSort = function(groups, groupId, groupOrder) {
    var groupData = groups.get(groupId);
    if (!groupData.nestedInGroup) {
      newGroupIdsOrder.push(groupId)
    }
    //must recursively sort groups - jobrian
    if (groupData.nestedGroups) {
      var nestedGroups = groups.get({
      ...
      for (var j = 0; j < nestedGroupIds.length; j++) {
        newGroupIdsOrder.push(nestedGroupIds[j]);   
        treeSort(groups, nestedGroupIds[j], groupOrder);
      }
  ...
  for  (var i=0; i < groupIds.length; i++) {
      treeSort(this.groupsData, groupIds[i], this.options.groupOrder);
  }
...
//      });
//      var nestedGroupIds = nestedGroups.map(function(nestedGroup) { return nestedGroup.id })
//      //You get the immediate child group, but not child of the child...
////      nestedGroupIds.forEach
//      newGroupIdsOrder = newGroupIdsOrder.concat(nestedGroupIds);
//    }
//  }, this)
...
ItemSet.prototype._onGroupClick = function (event) {
  ...
  var fullNestedGroups = [];

  var nestingGroup = groupsData.get(group.groupId)
  nestingGroup.showNested = !nestingGroup.showNested;

  var cascadeGroup = function (nestedIds) {
    var nestedGroups = groupsData.get(nestedIds).map(function(nestedGroup) {
    ...
    for (var j=0; j < nestedGroups.length; j++) {
        var child = nestedGroups[j];
        fullNestedGroups.unshift(child);
        if (child.nestedGroups) {
            cascadeGroup(child.nestedGroups);
        }
    }
  }
  
  for (var i = 0; i < group.nestedGroups.length; i++) {
//      fullNestedGroups.unshift(groupsData.get(group.nestedGroups[i]));
      cascadeGroup(group.nestedGroups);
  }
  fullNestedGroups.push(nestingGroup);
  groupsData.update(fullNestedGroups);
